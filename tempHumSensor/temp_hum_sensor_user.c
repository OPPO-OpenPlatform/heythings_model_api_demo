#include "schema/tempHumSensor/temp_hum_sensor_service.h"
#include "heythings.h"
...
int32_t temp_hum_sensor_properties_write_cb(size_t count, uint32_t *enumerate, struct temp_hum_sensor_properties *properties)
{
    for (size_t i = 0; i < count; i++) {
        switch (enumerate[i]) {
        case TEMP_HUM_SENSOR_PROPERTY_CURRENTTEMPERATURE:
            hey_printf(HEY_LOG_LEVEL_NOTICE, "current Temperature = %d", properties->currenttemperature);
            break;
        case TEMP_HUM_SENSOR_PROPERTY_CURRENTHUMIDITY:
            hey_printf(HEY_LOG_LEVEL_NOTICE, "current Humidity = %d", properties->currenthumidity);
            break;
        case TEMP_HUM_SENSOR_PROPERTY_ENTHALPY:
            hey_printf(HEY_LOG_LEVEL_NOTICE, "enthalpy = %d", properties->enthalpy);
            break;
        default:
            break;
        }
    ...
    return temp_hum_sensor_properties_update(count, enumerate, properties);
}
